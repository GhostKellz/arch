#!/bin/bash
# ghost-kernel-build.sh - Safe Kernel + NVIDIA DKMS Build w/ Recovery + Bootloader
# Author: GhostKellz

set -euo pipefail

### === LOGGING ===
LOGFILE="$HOME/ghostctl/logs/kernel-install-$(date +%F-%H%M).log"
mkdir -p "$(dirname "$LOGFILE")"
exec > >(tee -a "$LOGFILE") 2>&1

### === CONFIGURATION ===
KERNEL_NAME="linux-ghost"
...
if [ "$SNAPSHOT_BEFORE_INSTALL" = true ]; then
    ...
    sudo snapper create --description "Pre-GhostCTL Kernel + NVIDIA Install" || echo "⚠️ Snapper snapshot failed (non-fatal)."
fi
...
cp -r "$LINUX_TKG_DIR/linux615rc2-tkg-userpatches" "$BACKUP_DIR/userpatches" || echo "⚠️ No userpatches directory found"
sudo cp "$KERNEL_ENTRY" "$BACKUP_DIR/loader.conf.bak" || echo "⚠️ Failed to back up loader entry"
...
if [[ -z "$PARTUUID" ]]; then
    echo "❌ Could not find PARTUUID! Aborting."
    exit 1
fi

sudo tee "$KERNEL_ENTRY" >/dev/null <<EOF
# Custom Linux Kernel (BORE)
title   Linux Ghost 6.15 RC2
