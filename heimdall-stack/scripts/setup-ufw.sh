...
# Documentation ranges: replace these with the deployment's actual CIDRs.
LAN="192.0.2.0/24"       # primary LAN
DMZ="198.51.100.0/24"    # public-edge segment
...
# --- Admin UIs: trusted LAN only; Tailscale is allowed above ---
ufw allow from "$LAN" to any port 3000 proto tcp comment 'grafana'
ufw allow from "$LAN" to any port 9090 proto tcp comment 'prometheus'
ufw allow from "$LAN" to any port 9093 proto tcp comment 'alertmanager'
