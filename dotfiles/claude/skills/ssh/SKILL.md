...
## Host Reference (template)

Keep a per-host cheat sheet locally (host-specific values stay out of this
public mirror). A useful entry records:

- Host / role (e.g. Ubuntu VM, self-hosted CI runner).
- Login user and its groups (`sudo`, `docker`) — reuse the existing user, don't
  spin up a parallel runner user.
- Key path (e.g. `/home/<you>/.ssh/id_ed25519`) — path only, never the key.
- Service unit name to check after changes.
- **Sudo mode:** on some hosts password auth works only through a PTY — use the
  `printf ... | ssh -tt ... 'sudo -S -p "" -k ...'` pattern above. A non-PTY
  `sudo -S` failure there is a method problem, not a password problem.
